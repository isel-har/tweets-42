...
#________________________________________________________
    ...
    def spelling(cls, tokenized_tweets: list):
        ...
        for tokens in tokenized_tweets:
            ...
            for word in tokens:
                ...
                corrected_tokens.append(corrected_word if corrected_word is not None else '')
    ...
    def wordcounts(self, standardized_tokens: list):

        self.vectorizer = CountVectorizer(
            binary=(self.vectorization == 'binary'),
    ...
    def vectorize(self, standardized_tokens: list):

        if self.vectorization in ('bow', 'binary'):
            return self.wordcounts(standardized_tokens)
    ...
    #     if self.vectorization not in self.vectorization_methods:
    #         raise Exception(f'{self.vectorization} invalid vectorization.')
    ...
    #     vectors = self.vectorize(tokenized_sentences)
    #     self.processed = True
    #     return vectors

#############################################################################################################3

    def transform_new(self, new_tokens: list):
        if self.vectorizer is None:
            raise ValueError("Vectorizer not fitted yet. Call fit_vectorizer first.")
        new_vector = self.vectorizer.transform(new_tokens)

        return np.array(new_vector.toarray())
    
    def process_sentences(self, sentences:list):
        if self.processed == False:
            raise Exception("data preprocessing required before.")

        cleaned_sentences   = self.clean(sentences)
        tokenized_sentences = self.tokenize(cleaned_sentences)

        return self.transform_new(tokenized_sentences)
